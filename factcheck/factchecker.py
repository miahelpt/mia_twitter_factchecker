...
class MiaFactChecker():
    def __init__(self):
        ...
        self.index = lookup.embedding.FaissIndex(index_size=self.embed.dimensions, name="combined", embedding=self.embed.name) #we use the embeddings index size
        ...
        if(len(fr_sentences)> 0):
            for fr_sentence in fr_sentences:
                ...
                try:
                    for i in I:
                        for c in i:
                            matched_claim = dfSentences.loc[dfSentences['id'] == c]
                            claim_id = matched_claim["id"].item()
                            claim_text = matched_claim["text"].item()

                            simil = self.crossEnc.predict([fr_to_check, claim_text])

                            if  simil>0.3: #d[it][itr]>=0.5:
                                related_factchecks =  dfFactchecks.loc[dfFactchecks['id'] == c]
